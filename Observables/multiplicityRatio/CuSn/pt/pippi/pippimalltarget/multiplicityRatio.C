...
void multiplicityRatio() {
    ...
    auto* h_LD2_ele = new TH1F("h_LD2_ele", "LD Electrons;nu;Counts", nBins, pt2Min, pt2Max);
    auto* h_LD2_piP = new TH1F("h_LD2_piP", "LD Positive Pions;nu;Counts", nBins, pt2Min, pt2Max);
    auto* h_LD2_piM = new TH1F("h_LD2_piM", "LD Negative Pions;nu;Counts", nBins, pt2Min, pt2Max);
    auto* h_Cu_ele = new TH1F("h_Cu_ele", "Copper Electrons;nu;Counts", nBins, pt2Min, pt2Max);
    auto* h_Cu_piP = new TH1F("h_Cu_piP", "Copper Positive Pions;nu;Counts", nBins, pt2Min, pt2Max);
    auto* h_Cu_piM = new TH1F("h_Cu_piM", "Copper Negative Pions;nu;Counts", nBins, pt2Min, pt2Max);
    auto* h_Sn_ele = new TH1F("h_Sn_ele", "Tin Electrons;nu;Counts", nBins, pt2Min, pt2Max);
    auto* h_Sn_piP = new TH1F("h_Sn_piP", "Tin Positive Pions;nu;Counts", nBins, pt2Min, pt2Max);
    auto* h_Sn_piM = new TH1F("h_Sn_piM", "Tin Negative Pions;nu;Counts", nBins,pt2Min, pt2Max);
    // Add histograms for Carbon target (positive and negative pions)
    auto* h_Carbon_ele = new TH1F("h_Carbon_ele", "Carbon Electrons;nu;Counts", nBins, pt2Min, pt2Max);
    auto* h_Carbon_piP = new TH1F("h_Carbon_piP", "Carbon Positive Pions;nu;Counts", nBins, pt2Min, pt2Max);
    auto* h_Carbon_piM = new TH1F("h_Carbon_piM", "Carbon Negative Pions;nu;Counts", nBins, pt2Min, pt2Max);
...
while (chainC.Next()) {
    ...
    for (auto& p : particles) {
     ...
     if (foundElectron_Carbon && foundPionP_Carbon) {
            //double z = pip_temp_Carbon.E() / nu_Carbon;
            double Pt2_Carbon = CalculatePt2(pip_temp_Carbon, q_Carbon); 
            h_Carbon_piP->Fill(Pt2_Carbon);
        }
        if (foundElectron_Carbon && foundPionM_Carbon) {
            //double z = pim_temp_Carbon.E() / nu_Carbon;
            double Pt2_Carbon = CalculatePt2(pim_temp_Carbon, q_Carbon); 
            h_Carbon_piM->Fill(Pt2_Carbon);
        }
}
    ...
    while (chainCuSn.Next()) {
        ...
        TLorentzVector el_temp_Cu, pip_temp_Cu, pim_temp_Cu, el_temp_Sn, pip_temp_Sn, pim_temp_Sn;

        bool foundElectron_Cu = false;
        bool foundElectron_Sn = false;
        bool foundPionP_Cu = false;
        bool foundPionM_Cu = false;
        bool foundPionP_Sn = false;
        bool foundPionM_Sn = false;
        ...
        for (auto& p : particles) {
            ...
            if (Vz >= -11.463 && Vz <= -6.576) { // Cu target
                if (pid == 11 && status < 0 && chi2Pid < 5 && chi2Pid > -5) {
                    SetLorentzVector(el_temp_Cu, p);
                    foundElectron_Cu = true;
                    nu_Cu = beam.Energy() - el_temp_Cu.Energy();
                    q_Cu = beam - el_temp_Cu;
                    Q2_Cu = -q_Cu.Mag2(); 
                    Cu_ele++;
                }

                if (pid == 211 && chi2Pid > -10 && chi2Pid < 10) {
                    SetLorentzVector(pip_temp_Cu, p);
                    foundPionP_Cu = true;
                   // h_Cu_piP->Fill(Q2_Cu);
                }

                if (pid == -211 && chi2Pid > -10 && chi2Pid < 10) {
                    SetLorentzVector(pim_temp_Cu, p);
                    foundPionM_Cu = true;
                    //h_Cu_piM->Fill(Q2_Cu);
                }
            } else if (Vz >= -6.137 && Vz <= 5) { // Sn target
                if (pid == 11 && status < 0 && chi2Pid < 5 && chi2Pid > -5) {
                    SetLorentzVector(el_temp_Sn, p);
                    foundElectron_Sn = true;
                    nu_Sn = beam.Energy() - el_temp_Sn.Energy();
                    q_Sn = beam - el_temp_Sn;
                    Q2_Sn = -q_Sn.Mag2();
                    Sn_ele++;
                }

                if (pid == 211 && chi2Pid > -10 && chi2Pid < 10) {
                    SetLorentzVector(pip_temp_Sn, p);
                    foundPionP_Sn = true;
                    //h_Sn_piP->Fill(Q2_Sn);
                }

                if (pid == -211 && chi2Pid > -10 && chi2Pid < 10) {
                    SetLorentzVector(pim_temp_Sn, p);
                    foundPionM_Sn = true;
                    //h_Sn_piM->Fill(Q2_Sn);
                }
            }
         ...
         if (foundElectron_Cu && foundPionP_Cu) {
            //double z_Cu = pip_temp_Cu.E() / nu_Cu;
            double Pt2_Cu = CalculatePt2(pip_temp_Cu, q_Cu);
            h_Cu_piP->Fill(Pt2_Cu);
        }
        if (foundElectron_Cu && foundPionM_Cu) {
            //double z_Cu = pim_temp_Cu.E() / nu_Cu;
            double Pt2_Cu = CalculatePt2(pim_temp_Cu, q_Cu);
            h_Cu_piM->Fill(Pt2_Cu);
        }
        if (foundElectron_Sn && foundPionP_Sn) {
           // double z_Sn = pip_temp_Sn.E() / nu_Sn;
           double Pt2_Sn = CalculatePt2(pip_temp_Sn, q_Sn);
            h_Sn_piP->Fill(Pt2_Sn);
        }
        if (foundElectron_Sn && foundPionM_Sn) {
            //double z_Sn = pim_temp_Sn.E() / nu_Sn;
            double Pt2_Sn = CalculatePt2(pim_temp_Sn, q_Sn);
            h_Sn_piM->Fill(Pt2_Sn);
        }
...
for (int bin = 1; bin <= nBins; ++bin) {
    ...
    if (LD2_ele > 0 && Cu_ele > 0 && count_LD2_piP > 0 && count_Cu_piP > 0) {
        R_h_A_piP_Cu = (count_Cu_piP / Cu_ele) / (count_LD2_piP / LD2_ele);
        error_R_h_A_piP_Cu = R_h_A_piP_Cu * sqrt(1.0 / count_Cu_piP + 1.0 / Cu_ele + 1.0 / count_LD2_piP + 1.0 / LD2_ele);
    ...
    if (LD2_ele > 0 && Sn_ele > 0 && count_LD2_piP > 0 && count_Sn_piP > 0) {
        R_h_A_piP_Sn = (count_Sn_piP / Sn_ele) / (count_LD2_piP / LD2_ele);
        error_R_h_A_piP_Sn = R_h_A_piP_Sn * sqrt(1.0 / count_Sn_piP + 1.0 /Sn_ele + 1.0 / count_LD2_piP + 1.0 / LD2_ele);
    ...
    if (LD2_ele > 0 && Carbon_ele > 0 && count_LD2_piP > 0 && count_Carbon_piP > 0) {
        R_h_A_piP_Carbon = (count_Carbon_piP / Carbon_ele) / (count_LD2_piP / LD2_ele);
        error_R_h_A_piP_Carbon = R_h_A_piP_Carbon * sqrt(1.0 / count_Carbon_piP + 1.0 / Carbon_ele + 1.0 / count_LD2_piP + 1.0 / LD2_ele);
...
TCanvas* canvas_piP = new TCanvas("canvas_multiplicity_piP", "Multiplicity Ratio vs P_{T}^{2} for Carbon, Copper, and Tin", 800, 600);
graph_multiplicity_piP_Cu->SetTitle("Multiplicity Ratio vs P_{T}^{2} for Carbon, Copper, and Tin");
graph_multiplicity_piP_Cu->GetXaxis()->SetTitle("P_{T}^{2} (GeV^{2})");
...
for (int bin = 1; bin <= nBins; ++bin) {
    ...
    // Calculate multiplicity ratio R_h_A for Cu
    if (LD2_ele > 0 && Cu_ele > 0 && count_LD2_piM > 0 && count_Cu_piM > 0) {
        R_h_A_piM_Cu = (count_Cu_piM / Cu_ele) / (count_LD2_piM / LD2_ele);
        error_R_h_A_piM_Cu = R_h_A_piM_Cu * sqrt(1.0 / count_Cu_piM + 1.0 / Cu_ele + 1.0 / count_LD2_piM + 1.0 / LD2_ele);
    ...
    // Calculate multiplicity ratio R_h_A for Sn
    if (LD2_ele > 0 && Sn_ele > 0 && count_LD2_piM > 0 && count_Sn_piM > 0) {
        R_h_A_piM_Sn = (count_Sn_piM / Sn_ele) / (count_LD2_piM / LD2_ele);
        error_R_h_A_piM_Sn = R_h_A_piM_Sn * sqrt(1.0 / count_Sn_piM + 1.0 / Sn_ele + 1.0 / count_LD2_piM + 1.0 / LD2_ele);
    ...
    // Calculate multiplicity ratio R_h_A for Carbon
    if (LD2_ele > 0 && Carbon_ele > 0 && count_LD2_piM > 0 && count_Carbon_piM > 0) {
        R_h_A_piM_Carbon = (count_Carbon_piM / Carbon_ele) / (count_LD2_piM / LD2_ele);
        error_R_h_A_piM_Carbon = R_h_A_piM_Carbon * sqrt(1.0 / count_Carbon_piM + 1.0 / Carbon_ele + 1.0 / count_LD2_piM + 1.0 / LD2_ele);
...
// Create a canvas for the pi- graph
TCanvas* canvas_piM = new TCanvas("canvas_multiplicity_piM", "Multiplicity Ratio vs P_{T}^{2} for Carbon, Copper, and Tin", 800, 600);
graph_multiplicity_piM_Cu->SetTitle("Multiplicity Ratio vs P_{T}^{2} for Carbon, Copper, and Tin");
graph_multiplicity_piM_Cu->GetXaxis()->SetTitle("P_{T}^{2} (GeV^{2})");
