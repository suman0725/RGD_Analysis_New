...
void multiplicityRatio() {
    ...
    auto* h_LD2_piP = new TH1F("h_LD2_piP", "LD Positive Pions;z;Counts", nBins, zMin, zMax);
    auto* h_C_piP = new TH1F("h_C_piP", "Carbon Positive Pions;z;Counts", nBins, zMin, zMax);

    // Histograms for π⁻ (negative pion)
    auto* h_LD2_piM = new TH1F("h_LD2_piM", "LD Negative Pions;z;Counts", nBins, zMin, zMax);
    auto* h_C_piM = new TH1F("h_C_piM", "Carbon Negative Pions;z;Counts", nBins, zMin, zMax);
...
graphP->SetTitle("Multiplicity Ratio for #pi^{+} and #pi^{-};z;R_{h}^{A}");
