...
void plotBetaMTNormalizeLow_16() {
    TStopwatch timer;
    timer.Start();
    ...
    canvas->Print("beta_histograms_mt_normalize_cherenkov_16.pdf[");
    ...
    for (float pLow = 2.0; pLow < pMax; pLow += pStep) {
        ...
        // Histogram for the beta distribution with x-axis from 0.88 to 1.02
        TH1F* hist = new TH1F(Form("hist_beta_%.2f_%.2f", pLow, pHigh),
                              Form("Beta Distribution for %.2f < p < %.2f", pLow, pHigh),
                              200, 0.88, 1.02);  // x-axis range from 0.88 to 1.02
        ...
        // Set the maximum y-axis value (for better zooming)
        hist->SetMaximum(0.03);
        
        hist->SetLineWidth(0);  // Remove the line around the markers
        hist->SetLineColor(0);
         // Set the point color, style, and size
        hist->SetMarkerColor(kBlue); // Set marker color (red)
        hist->SetMarkerStyle(20);   // Set marker style (20 for circular points)
        hist->SetMarkerSize(0.8);     // Set marker size
        //hist->SetStats(kFALSE); 
        // Draw the histogram as points only (no line)
        hist->Draw("P");  // "P" for points onlyne
        canvas->Print("beta_histograms_mt_normalize_cherenkov_16.pdf");
    ...
    timer.Stop();
    cout << "Histograms saved to beta_histogram_mt_normalize_cherenkov_16.pdf" << endl;
