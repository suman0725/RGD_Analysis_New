    ...
    */
    ...
    for (int i = 0; i < nBins; i++) {
        ...
        */
        ...
        double pionMeanBefore, pionSigmaBefore, pionChi2Min, pionChi2Max, pionAmpBefore;
        ...
        chi2PionsBefore->Fit(cbPionBefore, "R", "", -10, 10);
        ...
        // commented out this gaussian fit to beta pions before cut
       ...
       // Using double crystal ball fit to beta pions before cut
       if (betaPionsBefore->GetEntries() >= 10) {
        TF1* gausPionBefore = new TF1(TString::Format("gaus_%s", betaPionsBefore->GetName()), "gaus", betaFitRanges[i].first, betaFitRanges[i].second);
        if (i == 9) {
            gausPionBefore->SetParameters(betaPionsBefore->GetMaximum(), 0.996, 0.004);
        ...
        // Double Crystal Ball fit for pions beta before
        TF1* cbPionBetaBefore = new TF1("cbFit", doubleCrystalBall, betaFitRanges[i].first, betaFitRanges[i].second, 8);
        cbPionBetaBefore->SetParameters(
            pConstBefore * 0.7, // A_left
            pMeanBefore,        // mu
            pSigmaBefore,       // sigma
            1.5,                // alpha_left
            2.0,                // n_left
            pConstBefore * 0.3, // A_right
            1.5,                // alpha_right
            2.0                 // n_right
        );
        cbPionBetaBefore->SetParLimits(0, 0, pConstBefore * 1.2);
        cbPionBetaBefore->SetParLimits(1, pMeanBefore - 2 * pSigmaBefore, pMeanBefore + 2 * pSigmaBefore);
        cbPionBetaBefore->SetParLimits(2, pSigmaBefore * 0.5, pSigmaBefore * 2.0);
        cbPionBetaBefore->SetParLimits(3, 0.5, 5.0);
        cbPionBetaBefore->SetParLimits(4, 1.0, 30.0);
        cbPionBetaBefore->SetParLimits(5, 0, pConstBefore * 0.6);
        cbPionBetaBefore->SetParLimits(6, 0.5, 5.0);
        cbPionBetaBefore->SetParLimits(7, 1.0, 30.0);
        betaPionsBefore->Fit(cbPionBetaBefore, "R", "", betaFitRanges[i].first, betaFitRanges[i].second);
        pMeanBefore = cbPionBetaBefore->GetParameter(1);
        pSigmaBefore = cbPionBetaBefore->GetParameter(2);
        pConstBefore = cbPionBetaBefore->GetParameter(0) + cbPionBetaBefore->GetParameter(5);
    }
        ...
        if (betaPionsBefore->GetEntries() >= 10) {
            ...
            betaPionsBefore->Draw("P");
            yMax = betaPionsBefore->GetMaximum();
            if (auto* fit = betaPionsBefore->GetFunction("cbFit")) {
                fit->SetLineColor(kBlue);
                fit->SetLineStyle(kDashed);
                fit->Draw("SAME");
                TLegend* leg = new TLegend(0.6, 0.6, 0.9, 0.9);
                leg->AddEntry(betaPionsBefore, "Pions", "p");
                leg->AddEntry(fit, "Double CB Fit", "l");
                leg->AddEntry((TObject*)0, TString::Format("A_{left}: %.5f", fit->GetParameter(0)), "");
                leg->AddEntry((TObject*)0, TString::Format("#mu: %.5f", fit->GetParameter(1)), "");
                leg->AddEntry((TObject*)0, TString::Format("#sigma: %.5f", fit->GetParameter(2)), "");
                leg->AddEntry((TObject*)0, TString::Format("#alpha_{left}: %.5f", fit->GetParameter(3)), "");
                leg->AddEntry((TObject*)0, TString::Format("n_{left}: %.5f", fit->GetParameter(4)), "");
                leg->AddEntry((TObject*)0, TString::Format("A_{right}: %.5f", fit->GetParameter(5)), "");
                leg->AddEntry((TObject*)0, TString::Format("#alpha_{right}: %.5f", fit->GetParameter(6)), "");
                leg->AddEntry((TObject*)0, TString::Format("n_{right}: %.5f", fit->GetParameter(7)), "");
                leg->SetBorderSize(0);
                leg->SetFillStyle(0);
       ...
       /*  if (betaPionsAfter->GetEntries() >= 10) {
        ...
        if (betaPionsAfter->GetEntries() >= 10) {
            ...
            // Double Crystal Ball fit for pions beta after
            TF1* cbPionBetaAfter = new TF1("cbFit", doubleCrystalBall, betaFitRanges[i].first, betaFitRanges[i].second, 8);
            cbPionBetaAfter->SetParameters(
                pConstAfter * 0.7, // A_left
                pMeanAfter,        // mu
                pSigmaAfter,       // sigma
                1.5,               // alpha_left
                2.0,               // n_left
                pConstAfter * 0.3, // A_right
                1.5,               // alpha_right
                2.0                // n_right
            );
            cbPionBetaAfter->SetParLimits(0, 0, pConstAfter * 1.2);
            cbPionBetaAfter->SetParLimits(1, pMeanAfter - 2 * pSigmaAfter, pMeanAfter + 2 * pSigmaAfter);
            cbPionBetaAfter->SetParLimits(2, pSigmaAfter * 0.5, pSigmaAfter * 2.0);
            cbPionBetaAfter->SetParLimits(3, 0.5, 5.0);
            cbPionBetaAfter->SetParLimits(4, 1.0, 30.0);
            cbPionBetaAfter->SetParLimits(5, 0, pConstAfter * 0.6);
            cbPionBetaAfter->SetParLimits(6, 0.5, 5.0);
            cbPionBetaAfter->SetParLimits(7, 1.0, 30.0);
            betaPionsAfter->Fit(cbPionBetaAfter, "R", "", betaFitRanges[i].first, betaFitRanges[i].second);
            pMeanAfter = cbPionBetaAfter->GetParameter(1);
            pSigmaAfter = cbPionBetaAfter->GetParameter(2);
            pConstAfter = cbPionBetaAfter->GetParameter(0) + cbPionBetaAfter->GetParameter(5);
        } 
        ...
        if (betaPionsAfter->GetEntries() >= 10) {
            ...
            betaPionsAfter->Draw("P");
            yMax = betaPionsAfter->GetMaximum();
            if (auto* fit = betaPionsAfter->GetFunction("cbFit")) {
                fit->SetLineColor(kBlue);
                fit->SetLineStyle(kDashed);
                //fit->SetRange(0.95, 1.03);
                fit->Draw("SAME");
