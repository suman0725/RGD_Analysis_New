...
int main() {
    ...
    for (int i = 5; i <= 5; i++) {
        ...
        if (betaPionsAfter->GetEntries() >= 10) {
            gausPion = new TF1(TString::Format("gaus_%p_%d", betaPionsAfter, i), "gaus", betaFitRanges[i].first, betaFitRanges[i].second);
            gausPion->SetParameters(betaPionsAfter->GetMaximum(), 1.0, 0.005);
            betaPionsAfter->Fit(gausPion, "R", "", 0.98, 1.01); // Adjusted range for bin 5
            pMeanAfter = gausPion->GetParameter(1);
            pSigmaAfter = gausPion->GetParameter(2);
            pConstAfter = gausPion->GetParameter(0);
            // Double Crystal Ball fit for pions beta after
            cbPionBetaAfter = new TF1("cbPionBetaAfter", doubleCrystalBall, 0.98, 1.01, 8);
            cbPionBetaAfter->SetParameters(
                pConstAfter * 0.7, // A_left
                pMeanAfter,        // mu
                pSigmaAfter,       // sigma
                1.0,               // alpha_left (adjusted for bin 5)
                5.0,               // n_left (softer left tail)
                pConstAfter * 0.3, // A_right
                1.0,               // alpha_right
                10.0               // n_right (steeper right tail)
            );
            ...
            cbPionBetaAfter->SetParLimits(5, 0, pConstAfter * 0.5);
            cbPionBetaAfter->SetParLimits(6, 0.5, 5.0);
            cbPionBetaAfter->SetParLimits(7, 5.0, 50.0);
            betaPionsAfter->Fit(cbPionBetaAfter, "R", "", 0.98, 1.01);
