...
int main() {
    ...
    ROOT::RDF::TH1DModel modelBeta("beta_fit_cb", "p: [4.3-4.7) GeV/c; #beta; Counts", 200, 0.95, 1.03);
    ...
    Double_t initParams[21] = {
        ...
        amp_K, beta_K_theory, 0.0015, 1.0, 2.0,             // Kaon
        amp_p, beta_p_theory, 0.003, 1.0, 2.0, 1.5, 2.0,   // Proton
        10.0, 5.0                                          // Background
    };
    ...
    // Pion parameters
    totalCB->SetParLimits(0, 0, 12000);
    ...
    // Proton parameters
    totalCB->SetParLimits(12, 0, 4000);
    totalCB->FixParameter(13, beta_p_theory);
    totalCB->SetParLimits(14, 0.002, 0.004);
    ...
    totalCB->SetParLimits(19, 0, 1000);
    totalCB->SetParLimits(20, -100, 100);
    ...
    double kaon_integral_after_dt = cb_kaon->Integral(0.99, 1.0099) / binWidth;
    double kaon_integral_error_after_dt = cb_kaon->IntegralError(0.99, 1.0099, totalCB->GetParameters(), fitResult->GetCovarianceMatrix().GetMatrixArray()) / binWidth;
    ...
    double proton_integral_after_dt = cb_proton->Integral(0.99, 1.0099) / binWidth;
    double proton_integral_error_after_dt = cb_proton->IntegralError(0.99, 1.0099, totalCB->GetParameters(), fitResult->GetCovarianceMatrix().GetMatrixArray()) / binWidth;
    ...
    double kaon_integral_after_chi2pid = cb_kaon->Integral(0.9959, 1.017) / binWidth;
    double kaon_integral_error_after_chi2pid = cb_kaon->IntegralError(0.9959, 1.017, totalCB->GetParameters(), fitResult->GetCovarianceMatrix().GetMatrixArray()) / binWidth;
    ...
    double proton_integral_after_chi2pid = cb_proton->Integral(0.995, 1.017) / binWidth;
    double proton_integral_error_after_chi2pid = cb_proton->IntegralError(0.995, 1.017, totalCB->GetParameters(), fitResult->GetCovarianceMatrix().GetMatrixArray()) / binWidth;
    ...
    hist_res->Draw();
    hist_res->Write("hist_res");
    c_res->Write("canvas_residuals");
    canvas->Write("canvas_fit");

  
    // Output to text file
    ofstream outFile("fit_parameters_bin11.txt");
    if (outFile.is_open()) {
        outFile << fixed << setprecision(4);
        outFile << "Fit Parameters for Crystal Ball Fit (Bin: p = 4.3 - 4.7 GeV/c)\n";
        outFile << "------------------------------------\n";
        ...
        cerr << "Error: Cannot open fit_parameters_bin11.txt for writing" << endl;
    }

    canvas->Print("bin_11.png");
    c_res->Print("residuals_bin10.png");
