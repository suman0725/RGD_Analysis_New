...
int main() {
    ...
    // Pion parameters
    totalCB->SetParLimits(0, 0, 120000);
    totalCB->FixParameter(1, beta_pi_theory);
    totalCB->SetParLimits(2, 0.002, 0.008);
    ...
    double kaon_integral_after_dt = cb_kaon->Integral(0.9865, 1.0072) / binWidth;
    ...
    double kaon_integral_after_chi2pid = cb_kaon->Integral(0.9782, 1.016) / binWidth;
    ...
    if (outFile.is_open()) {
        outFile << "Fit Parameters for Crystal Ball Fit (Bin: p = 1.6 - 1.9 GeV/c)\n";
        outFile << "------------------------------------\n";
    ...
    hist_res->Draw();
    c_res->Print("residuals_bin2.png");
