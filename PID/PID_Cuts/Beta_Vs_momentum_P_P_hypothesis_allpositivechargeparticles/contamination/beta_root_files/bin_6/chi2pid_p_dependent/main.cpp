...
int main() {
    ...
    ROOT::RDF::TH1DModel modelBeta("beta_fit_cb", "p: [2.8-3.1) GeV/c; #beta; Counts", 200, 0.95, 1.03);
    ...
    // Pion parameters
    totalCB->SetParLimits(0, 0, 37000);
    ...
    // Proton parameters
    totalCB->SetParLimits(12, 0, 20000);
    ...
    double kaon_integral_after_dt = cb_kaon->Integral(0.9898, 1.0085) / binWidth;
    double kaon_integral_error_after_dt = cb_kaon->IntegralError(0.9898, 1.0085, totalCB->GetParameters(), fitResult->GetCovarianceMatrix().GetMatrixArray()) / binWidth;
    ...
    double proton_integral_after_dt = cb_proton->Integral(0.989, 1.0085) / binWidth;
    double proton_integral_error_after_dt = cb_proton->IntegralError(0.989, 1.0085, totalCB->GetParameters(), fitResult->GetCovarianceMatrix().GetMatrixArray()) / binWidth;
    ...
    double kaon_integral_after_chi2pid = cb_kaon->Integral(0.991, 1.017) / binWidth;
    double kaon_integral_error_after_chi2pid = cb_kaon->IntegralError(0.991, 1.017, totalCB->GetParameters(), fitResult->GetCovarianceMatrix().GetMatrixArray()) / binWidth;
    ...
    double proton_integral_after_chi2pid = cb_proton->Integral(0.987, 1.017) / binWidth;
    double proton_integral_error_after_chi2pid = cb_proton->IntegralError(0.987, 1.017, totalCB->GetParameters(), fitResult->GetCovarianceMatrix().GetMatrixArray()) / binWidth;
    ...
    // Output to file
    ofstream outFile("fit_parameters_bin6.txt");
    if (outFile.is_open()) {
        outFile << "Fit Parameters for Crystal Ball Fit (Bin: p = 2.8 - 3.1 GeV/c)\n";
        outFile << "------------------------------------\n";
        ...
        cerr << "Error: Cannot open fit_parameters_bin6.txt for writing" << endl;
    }
    ...
    hist_res->Draw();
    c_res->Print("residuals_bin6.png");

    canvas->Print("bin_6.png");
