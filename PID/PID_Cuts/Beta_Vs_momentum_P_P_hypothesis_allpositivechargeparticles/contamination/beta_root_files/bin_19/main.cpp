...
int main() {
    ...
    ROOT::RDF::TH1DModel modelBeta("beta_fit_cb", "p: [6.7-7.0) GeV/c; #beta; Counts", 200, 0.95, 1.03);
    ...
    // Proton
    totalCB->SetParLimits(12, 0, 10000); totalCB->FixParameter(13, beta_p_theory); totalCB->SetParLimits(14, 0.002, 0.004);
    ...
    double kaonIntegral = cb_kaon->Integral(0.995, 1.03); // Adjusted for bin 6.7-7.0
    double protonIntegral = cb_proton->Integral(0.990, 1.03); // Adjusted for bin 6.7-7.0
    ...
    ofstream outFile("fit_parameters_bin67.txt");
    if (outFile.is_open()) {
        outFile << "Fit Parameters for Crystal Ball Fit (Bin 6: p = 6.7 - 7.0 GeV/c)\n";
        outFile << "------------------------------------\n";
        ...
        outFile << "------------------------------------\n";
        outFile << "Kaon Contamination (Kaon Integral [0.995, 1.03] / Pion Integral [0.95, 1.03]): " << fixed << setprecision(3) << kaonContamination << "%\n";
        outFile << "Proton Contamination (Proton Integral [0.990, 1.03] / Pion Integral [0.95, 1.03]): " << fixed << setprecision(3) << protonContamination << "%\n";
        outFile.close();
    } else {
        cerr << "Error: Cannot open fit_parameters_bin67.txt for writing" << endl;
    }
