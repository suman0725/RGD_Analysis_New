...
int main() {
    ...
    ROOT::RDF::TH1DModel modelBeta("beta_fit_cb", "p: [4.6-4.9) GeV/c; #beta; Counts", 200, 0.95, 1.03);
    ...
    cout << "Average momentum in the bin: " << p_mid << " GeV/c" << endl;
    cout << "Theoretical beta for pion: " << beta_pi_theory << endl;
    cout << "Theoretical beta for kaon: " << beta_K_theory << endl;
    cout << "Theoretical beta for proton: " << beta_p_theory << endl;
    ...
    Double_t initParams[15] = {
        amp_pi, beta_pi_theory, 0.003, 1.8, 3.0,          // Pion (amp, mean, sigma, alphaRight, nRight)
        amp_K, beta_K_theory, 0.002, 1.0, 2.0,          // Kaon (amp, mean, sigma, alpha, n)
        amp_p, beta_p_theory, 0.0025, 1.5, 2.0            // Proton (amp, mean, sigma, alphaRight, nRight)
    };
    ...
    // Pion
    totalCB->SetParLimits(0, 0, 9000); totalCB->SetParameter(1, beta_pi_theory); totalCB->SetParLimits(2, 0.002, 0.006);
    totalCB->SetParLimits(3, 0.1, 10.0); totalCB->SetParLimits(4, 1.0, 20.0);
    // Kaon
    totalCB->SetParLimits(5, 0, 2000); totalCB->SetParameter(6, beta_K_theory); totalCB->SetParLimits(7, 0.002, 0.003);
    totalCB->SetParLimits(8, 0.1, 20.0); totalCB->SetParLimits(9, 1.0, 10.0);
    // Proton
    totalCB->SetParLimits(10, 0, 3500); totalCB->SetParameter(11, beta_p_theory); totalCB->SetParLimits(12, 0.002, 0.006);
