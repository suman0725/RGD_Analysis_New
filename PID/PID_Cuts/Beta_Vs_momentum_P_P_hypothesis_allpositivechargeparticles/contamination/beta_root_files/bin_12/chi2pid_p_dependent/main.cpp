...
Double_t totalCrystalBallFunc(Double_t* x, Double_t* par) {
    ...
    Double_t cb3 = doubleSidedCrystalBallFuncSymm(x, &par[12]); // Proton
    Double_t bg = par[19] + par[20] * x[0];                 // Linear background
...
int main() {
    ...
    ROOT::RDF::TH1DModel modelBeta("beta_fit_cb", "p: [4.7-5.0) GeV/c; #beta; Counts", 200, 0.95, 1.03);
    ...
    // Fit setup
    TF1* totalCB = new TF1("totalCB", totalCrystalBallFunc, 0.95, 1.03, 21);
    totalCB->SetNpx(1000);
    totalCB->SetRange(0.955, 1.03);
    Double_t initParams[21] = {
        amp_pi, beta_pi_theory, 0.003, 2.0, 3.0, 2.0, 3.0, // Pion
        amp_K, beta_K_theory, 0.002, 1.0, 2.0,             // Kaon
        amp_p, beta_p_theory, 0.003, 1.5, 2.0, 2.0, 3,   // Proton
        10.0, 5.0                                          // Background
    };
    ...
    // Kaon parameters
    totalCB->SetParLimits(7, 0, 1500);
    totalCB->FixParameter(8, beta_K_theory);
    totalCB->SetParLimits(9, 0.0025, 0.003);
    totalCB->SetParLimits(10, 0.1, 10.0);
    totalCB->SetParLimits(11, 1.0, 20.0);

    // Proton parameters
    totalCB->SetParLimits(12, 0, 2000);
    totalCB->SetParameter(13, beta_p_theory);
    totalCB->SetParLimits(14, 0.002, 0.006);
    totalCB->SetParLimits(15, 0.1, 10.0);
    totalCB->SetParLimits(16, 1.0, 20.0);
    totalCB->SetParLimits(17, 0.1, 20.0);
    totalCB->SetParLimits(18, 1.0, 50.0);

    // Background parameters
    totalCB->SetParLimits(19, 0, 100);
    totalCB->SetParLimits(20, -50, 50);
    ...
    for (int i = 7; i < 19; ++i) pionParams[i] = 0;
    pionParams[19] = totalCB->GetParameter(19);
    pionParams[20] = totalCB->GetParameter(20);
    ...
    for (int i = 12; i < 19; ++i) kaonParams[i] = 0;
    kaonParams[19] = totalCB->GetParameter(19);
    kaonParams[20] = totalCB->GetParameter(20);
    ...
    for (int i = 12; i < 19; ++i) protonParams[i] = totalCB->GetParameter(i);
    for (int i = 0; i < 12; ++i) protonParams[i] = 0;
    protonParams[19] = totalCB->GetParameter(19);
    protonParams[20] = totalCB->GetParameter(20);
    ...
    bg->SetNpx(1000);
    bg->SetParameter(0, totalCB->GetParameter(19));
    bg->SetParameter(1, totalCB->GetParameter(20));
    ...
    double kaon_integral_after_dt = cb_kaon->Integral(0.99,1.0098 ) / binWidth;
    double kaon_integral_error_after_dt = cb_kaon->IntegralError(0.99,1.0098, totalCB->GetParameters(), fitResult->GetCovarianceMatrix().GetMatrixArray()) / binWidth;
    ...
    double proton_integral_after_dt = cb_proton->Integral(0.991, 1.0098) / binWidth;
    double proton_integral_error_after_dt = cb_proton->IntegralError(0.991, 1.0098, totalCB->GetParameters(), fitResult->GetCovarianceMatrix().GetMatrixArray()) / binWidth;
    if (proton_integral_error_after_dt <= 0 || !fitResult->IsValid()) {
        cout << "Warning: Proton integral error after (dt cut) is zero or negative, using sqrt(N) approximation" << endl;
        proton_integral_error_after_dt = sqrt(proton_integral_after_dt);
    }
    ...
    double proton_contamination_dt = proton_integral_after_dt / total_after_dt;
    double proton_contamination_error_dt = proton_contamination_dt * sqrt(pow(proton_integral_error_after_dt / proton_integral_after_dt, 2) + pow(total_after_error_dt / total_after_dt, 2));
    ...
    double kaon_integral_after_chi2pid = cb_kaon->Integral(0.996, 1.017) / binWidth;
    double kaon_integral_error_after_chi2pid = cb_kaon->IntegralError(0.996, 1.017, totalCB->GetParameters(), fitResult->GetCovarianceMatrix().GetMatrixArray()) / binWidth;
    ...
    double proton_integral_after_chi2pid = cb_proton->Integral(0.996, 1.018) / binWidth;
    double proton_integral_error_after_chi2pid = cb_proton->IntegralError(0.996, 1.018, totalCB->GetParameters(), fitResult->GetCovarianceMatrix().GetMatrixArray()) / binWidth;
    if (proton_integral_error_after_chi2pid <= 0 || !fitResult->IsValid()) {
        cout << "Warning: Proton integral error after (chi2pid cut) is zero or negative, using sqrt(N) approximation" << endl;
        proton_integral_error_after_chi2pid = sqrt(proton_integral_after_chi2pid);
    }
    ...
    double proton_contamination_chi2pid = proton_integral_after_chi2pid / total_after_chi2pid;
    double proton_contamination_error_chi2pid = proton_contamination_chi2pid * sqrt(pow(proton_integral_error_after_chi2pid / proton_integral_after_chi2pid, 2) + pow(total_after_error_chi2pid / total_after_chi2pid, 2));

    // Output to text file
    ofstream outFile("fit_parameters_bin12.txt");
    if (outFile.is_open()) {
        outFile << fixed << setprecision(4);
        outFile << "Fit Parameters for Crystal Ball Fit (Bin: p = 4.7 - 5.0 GeV/c)\n";
        outFile << "------------------------------------\n";
        ...
        outFile << "------------------------------------\n";
        outFile << "Pion Efficiency and Contamination Results (dt Cut)\n";
        outFile << "--------------------------------\n";
        ...
        outFile << "Protons after dt cut: " << proton_integral_after_dt << " ± " << proton_integral_error_after_dt << "\n";
        ...
        outFile << "Proton Contamination (dt cut): " << proton_contamination_dt * 100 << " ± " << proton_contamination_error_dt * 100 << " %\n";
        outFile << "--------------------------------\n";
        outFile << "Pion Efficiency and Contamination Results (chi2pid Cut)\n";
        outFile << "--------------------------------\n";
        outFile << "Momentum range: [" << pLow << ", " << pHigh << "] GeV/c\n";
        outFile << "Average momentum: " << p_mid << " GeV/c\n";
        ...
        outFile << "Protons after chi2pid cut: " << proton_integral_after_chi2pid << " ± " << proton_integral_error_after_chi2pid << "\n";
        ...
        outFile << "Proton Contamination (chi2pid cut): " << proton_contamination_chi2pid * 100 << " ± " << proton_contamination_error_chi2pid * 100 << " %\n";
        outFile << "--------------------------------\n";
        outFile.close();
    } else {
        cerr << "Error: Cannot open fit_parameters_bin12.txt for writing" << endl;
    }
    ...
    canvas->Print("bin_12.png");
    c_res->Print("residuals_bin12.png");
