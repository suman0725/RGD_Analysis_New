...
int main() {
    ...
    Double_t initParams[21] = {
        amp_pi, beta_pi_theory, 0.004, 2.0, 3.0, 2.0, 3.0, // Pion
    ...
    totalCB->SetParLimits(2, 0.002, 0.005);
    ...
    // Proton parameters
    totalCB->SetParLimits(12, 0, 5000);
    ...
    double kaon_integral_after_dt = cb_kaon->Integral(0.99, 0.999) / binWidth;
    double kaon_integral_error_after_dt = cb_kaon->IntegralError(0.99, 0.999, totalCB->GetParameters(), fitResult->GetCovarianceMatrix().GetMatrixArray()) / binWidth;
    ...
    double proton_integral_after_dt = cb_proton->Integral(0.9899, 0.999) / binWidth;
    double proton_integral_error_after_dt = cb_proton->IntegralError(0.9899, 0.999, totalCB->GetParameters(), fitResult->GetCovarianceMatrix().GetMatrixArray()) / binWidth;
    ...
    double kaon_integral_after_chi2pid = cb_kaon->Integral(0.9945, 1.017) / binWidth;
    double kaon_integral_error_after_chi2pid = cb_kaon->IntegralError(0.9945, 1.017, totalCB->GetParameters(), fitResult->GetCovarianceMatrix().GetMatrixArray()) / binWidth;
    ...
    double proton_integral_after_chi2pid = cb_proton->Integral(0.994, 1.017) / binWidth;
    double proton_integral_error_after_chi2pid = cb_proton->IntegralError(0.994, 1.017, totalCB->GetParameters(), fitResult->GetCovarianceMatrix().GetMatrixArray()) / binWidth;
    ...
    if (outFile.is_open()) {
        outFile << "Fit Parameters for Crystal Ball Fit (Bin: p = 4.0 - 4.3 GeV/c)\n";
        outFile << "------------------------------------\n";
