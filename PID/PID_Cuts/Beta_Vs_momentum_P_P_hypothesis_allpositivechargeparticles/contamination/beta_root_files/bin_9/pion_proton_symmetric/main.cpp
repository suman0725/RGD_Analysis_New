...
// Double-sided Crystal Ball function for π+ (left tail mirrored from right)
...
Double_t totalCrystalBallFunc(Double_t* x, Double_t* par) {
    ...
    Double_t cb3 = doubleSidedCrystalBallFuncSymm(x, &par[10]); // Proton (independent double-sided)
    return cb1 + cb2 + cb3;
}
...
int main() {
    ...
    TF1* totalCB = new TF1("totalCB", totalCrystalBallFunc, 0.95, 1.03, 17); // 17 parameters (5 pion, 5 kaon, 7 proton)
    totalCB->SetNpx(1000);
    totalCB->SetRange(0.95, 1.02);
    Double_t initParams[17] = {
        amp_pi, beta_pi_theory, 0.003, 2.0, 3.0,          // Pion (amp, mean, sigma, alphaRight, nRight)
        amp_K, beta_K_theory, 0.0025, 1.0, 2.0,          // Kaon (amp, mean, sigma, alpha, n)
        amp_p, beta_p_theory, 0.003, 1.0, 2.0, 1.5, 2.0  // Proton (amp, mean, sigma, alphaLeft, nLeft, alphaRight, nRight)
    };
    ...
    // Kaon
    totalCB->SetParLimits(5, 0, 6000); totalCB->FixParameter(6, beta_K_theory); totalCB->SetParLimits(7, 0.0025, 0.004);
    totalCB->SetParLimits(8, 0.1, 20.0); totalCB->SetParLimits(9, 1.0, 100.0);
    // Proton
    totalCB->SetParLimits(10, 0, 10000); totalCB->FixParameter(11, beta_p_theory); totalCB->SetParLimits(12, 0.002, 0.006);
    totalCB->SetParLimits(13, 0.1, 10.0); totalCB->SetParLimits(14, 1.0, 20.0);
    totalCB->SetParLimits(15, 0.1, 10.0); totalCB->SetParLimits(16, 1.0, 50.0);
    ...
    TF1* cb_pion = new TF1("cb_pion", totalCrystalBallFunc, 0.95, 1.03, 17);
    cb_pion->SetNpx(1000);
    Double_t pionParams[17] = {0};
    for (int i = 0; i < 5; ++i) pionParams[i] = totalCB->GetParameter(i);
    for (int i = 5; i < 17; ++i) pionParams[i] = 0;
    cb_pion->SetParameters(pionParams);
    for (int i = 0; i < 17; ++i) cb_pion->FixParameter(i, pionParams[i]);
    cb_pion->SetLineColor(kBlue+2);
    cb_pion->SetLineWidth(2);
    cb_pion->Draw("SAME");

    // Kaon peak contribution
    TF1* cb_kaon = new TF1("cb_kaon", totalCrystalBallFunc, 0.95, 1.03, 17);
    cb_kaon->SetNpx(1000);
    Double_t kaonParams[17] = {0};
    for (int i = 5; i < 10; ++i) kaonParams[i] = totalCB->GetParameter(i);
    for (int i = 0; i < 5; ++i) kaonParams[i] = 0;
    for (int i = 10; i < 17; ++i) kaonParams[i] = 0;
    cb_kaon->SetParameters(kaonParams);
    for (int i = 0; i < 17; ++i) cb_kaon->FixParameter(i, kaonParams[i]);
    cb_kaon->SetLineColor(kGreen);
    cb_kaon->SetLineWidth(2);
    cb_kaon->Draw("SAME");

    // Proton peak contribution
    TF1* cb_proton = new TF1("cb_proton", totalCrystalBallFunc, 0.95, 1.03, 17);
    cb_proton->SetNpx(1000);
    Double_t protonParams[17] = {0};
    for (int i = 10; i < 17; ++i) protonParams[i] = totalCB->GetParameter(i);
    for (int i = 0; i < 10; ++i) protonParams[i] = 0;
    cb_proton->SetParameters(protonParams);
    for (int i = 0; i < 17; ++i) cb_proton->FixParameter(i, protonParams[i]);
...
int main() {
    ...
    for (int i = 0; i < 15; ++i)
        totalCB->SetParName(i, Form("p%d", i));
    // Pion
    totalCB->SetParLimits(0, 0, 16000); totalCB->SetParameter(1, beta_pi_theory); totalCB->SetParLimits(2, 0.002, 0.004);
    totalCB->SetParLimits(3, 0.1, 10.0); totalCB->SetParLimits(4, 1.0, 20.0);
    // Kaon
