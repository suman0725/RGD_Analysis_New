...
int main() {
    ...
    // Pion parameters
    totalCB->SetParLimits(0, 0, 16000);
    ...
    // Perform fit
    TFitResultPtr fitResult = histo_beta_before->Fit(totalCB, "RME+S", "", 0.95, 1.02);
    ...
    double proton_integral_after_dt = cb_proton->Integral(0.9888, 1.009) / binWidth;
    double proton_integral_error_after_dt = cb_proton->IntegralError(0.9888, 1.009, totalCB->GetParameters(), fitResult->GetCovarianceMatrix().GetMatrixArray()) / binWidth;
    ...
    double kaon_integral_after_chi2pid = cb_kaon->Integral(0.994, 1.017) / binWidth;
    double kaon_integral_error_after_chi2pid = cb_kaon->IntegralError(0.994, 1.017, totalCB->GetParameters(), fitResult->GetCovarianceMatrix().GetMatrixArray()) / binWidth;
    ...
    double proton_integral_after_chi2pid = cb_proton->Integral(0.993, 1.017) / binWidth;
    double proton_integral_error_after_chi2pid = cb_proton->IntegralError(0.993, 1.017, totalCB->GetParameters(), fitResult->GetCovarianceMatrix().GetMatrixArray()) / binWidth;
    ...
    if (outFile.is_open()) {
        outFile << "Fit Parameters for Crystal Ball Fit (Bin: p = 3.7 - 4.0 GeV/c)\n";
        outFile << "------------------------------------\n";
    ...
    hist_res->Draw();
    c_res->Print("residuals_bin9.png");
