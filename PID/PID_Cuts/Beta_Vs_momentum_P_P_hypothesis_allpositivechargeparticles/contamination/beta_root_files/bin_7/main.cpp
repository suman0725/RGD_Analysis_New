...
int main() {
    ...
    Double_t initParams[21] = {
        ...
        amp_p, beta_p_theory, 0.004, 1.0, 2.0, 1.5, 2.0,   // Proton (independent double-sided)
    ...
    // Pion
    totalCB->SetParLimits(0, 0, 30000); totalCB->FixParameter(1, beta_pi_theory); totalCB->SetParLimits(2, 0.002, 0.004);
    ...
    // Proton
    totalCB->SetParLimits(12, 0, 15000); totalCB->SetParameter(13, beta_p_theory); totalCB->SetParLimits(14, 0.002, 0.006);
    totalCB->SetParLimits(15, 0.3, 30.0); totalCB->SetParLimits(16, 31.0, 50.0);
    totalCB->SetParLimits(17, 0.1, 20.0); totalCB->SetParLimits(18, 1.0, 50.0);

    // Background
    totalCB->SetParLimits(19, 0, 1000); totalCB->SetParLimits(20, -100, 100);
    ...
    double kaonIntegral = cb_kaon->Integral(0.994, 1.03); // Adjusted for bin 3.4-3.7
    double protonIntegral = cb_proton->Integral(0.987, 1.03); // Adjusted for bin 3.4-3.7
    ...
    ofstream outFile("fit_parameters_bin34.txt");
    if (outFile.is_open()) {
        ...
        outFile << "------------------------------------\n";
        outFile << "Kaon Contamination (Kaon Integral [0.994, 1.03] / Pion Integral [0.95, 1.03]): " << fixed << setprecision(3) << kaonContamination << "%\n";
        outFile << "Proton Contamination (Proton Integral [0.987, 1.03] / Pion Integral [0.95, 1.03]): " << fixed << setprecision(3) << protonContamination << "%\n";
        outFile.close();
    } else {
        cerr << "Error: Cannot open fit_parameters_bin34.txt for writing" << endl;
    }
