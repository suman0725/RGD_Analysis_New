...
int main() {
    ...
    ROOT::RDF::TH1DModel modelBeta("beta_fit_cb", "p: [3.1-3.4) GeV/c; #beta; Counts", 200, 0.95, 1.03);
    ...
    Double_t initParams[21] = {
        ...
        amp_p, beta_p_theory, 0.004, 1.0, 2.0, 1.5, 2.0,   // Proton
        10.0, 5.0                                          // Background
    };
    ...
    // Pion parameters
    totalCB->SetParLimits(0, 0, 30000);
    totalCB->FixParameter(1, beta_pi_theory);
    totalCB->SetParLimits(2, 0.002, 0.004);
    ...
    double kaon_integral_after_dt = cb_kaon->Integral(0.99, 1.009) / binWidth;
    double kaon_integral_error_after_dt = cb_kaon->IntegralError(0.99, 1.009, totalCB->GetParameters(), fitResult->GetCovarianceMatrix().GetMatrixArray()) / binWidth;
    ...
    double proton_integral_after_dt = cb_proton->Integral(0.989, 1.009) / binWidth;
    double proton_integral_error_after_dt = cb_proton->IntegralError(0.989, 1.009, totalCB->GetParameters(), fitResult->GetCovarianceMatrix().GetMatrixArray()) / binWidth;
    ...
    double kaon_integral_after_chi2pid = cb_kaon->Integral(0.992, 1.017) / binWidth;
    double kaon_integral_error_after_chi2pid = cb_kaon->IntegralError(0.992, 1.017, totalCB->GetParameters(), fitResult->GetCovarianceMatrix().GetMatrixArray()) / binWidth;
    ...
    double proton_integral_after_chi2pid = cb_proton->Integral(0.99, 1.017 ) / binWidth;
    double proton_integral_error_after_chi2pid = cb_proton->IntegralError(0.99, 1.017, totalCB->GetParameters(), fitResult->GetCovarianceMatrix().GetMatrixArray()) / binWidth;
    ...
    // Output to file
    ofstream outFile("fit_parameters_bin7.txt");
    if (outFile.is_open()) {
        outFile << "Fit Parameters for Crystal Ball Fit (Bin: p = 3.1 - 3.4 GeV/c)\n";
        outFile << "------------------------------------\n";
        ...
        cerr << "Error: Cannot open fit_parameters_bin7.txt for writing" << endl;
    }
    ...
    hist_res->Draw();
    c_res->Print("residuals_bin7.png");

    canvas->Print("bin_7.png");
