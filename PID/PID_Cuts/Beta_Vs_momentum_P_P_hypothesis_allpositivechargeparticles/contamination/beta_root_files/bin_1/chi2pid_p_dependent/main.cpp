...
int main() {
    ...
    TFile* outputFile = new TFile("beta_histograms.root", "RECREATE");
    if (!outputFile || outputFile->IsZombie()) {
        cerr << "Error: Cannot create output file beta_histograms.root" << endl;
        return 1;
    }
    // Write histograms
    histo_beta_before->Write("histo_beta_before");
    histo_beta_after_dt->Write("histo_beta_after_dt");
    histo_beta_after_chi2pid->Write("histo_beta_after_chi2pid");
  ...
  // Determine the effective β range from the dt cut histogram
    ...
    bg->Draw("SAME");
        ...
        // Write fit functions
    totalCB->Write("totalCB");
    cb_pion->Write("cb_pion");
    cb_kaon->Write("cb_kaon");
    bg->Write("bg");
    ...
    double kaon_integral_after_dt = cb_kaon->Integral(0.9845, 1.006) / binWidth;
    double kaon_integral_error_after_dt = cb_kaon->IntegralError(beta_min_after_dt, beta_max_after_dt, totalCB->GetParameters(), fitResult->GetCovarianceMatrix().GetMatrixArray()) / binWidth;
    ...
    // Calculate efficiency and kaon contamination for dt cut
    ...
    double kaon_integral_after_chi2pid = cb_kaon->Integral(0.977, 1.018) / binWidth;
    double kaon_integral_error_after_chi2pid = cb_kaon->IntegralError(beta_min_after_chi2pid, beta_max_after_chi2pid, totalCB->GetParameters(), fitResult->GetCovarianceMatrix().GetMatrixArray()) / binWidth;
    ...
    // Calculate efficiency and kaon contamination for chi2pid cut
