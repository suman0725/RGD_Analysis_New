...
int main() {
    ...
    // Pion parameters
    totalCB->SetParLimits(0, 0, 25000);
    ...
    // Proton parameters
    totalCB->SetParLimits(12, 0, 10000);
    ...
    double kaon_integral_after_chi2pid = cb_kaon->Integral(0.993, 1.017) / binWidth;
    double kaon_integral_error_after_chi2pid = cb_kaon->IntegralError(0.993, 1.017, totalCB->GetParameters(), fitResult->GetCovarianceMatrix().GetMatrixArray()) / binWidth;
    ...
    double proton_integral_after_chi2pid = cb_proton->Integral(0.992, 1.017) / binWidth;
    double proton_integral_error_after_chi2pid = cb_proton->IntegralError(0.992, 1.017, totalCB->GetParameters(), fitResult->GetCovarianceMatrix().GetMatrixArray()) / binWidth;
    ...
    // Output to file
    ofstream outFile("fit_parameters_bin8.txt");
    if (outFile.is_open()) {
        outFile << "Fit Parameters for Crystal Ball Fit (Bin: p = 3.4 - 3.7 GeV/c)\n";
        outFile << "------------------------------------\n";
        ...
        cerr << "Error: Cannot open fit_parameters_bin8.txt for writing" << endl;
    }
    ...
    hist_res->Draw();
    c_res->Print("residuals_bin34.png");

    canvas->Print("bin_8.png");
