...
int main() {
    ...
    ROOT::RDF::TH1DModel modelBeta("beta_fit_cb", "p: [2.5-2.8) GeV/c; #beta; Counts", 200, 0.95, 1.03);
    ...
    // Pion parameters
    totalCB->SetParLimits(0, 0, 50000);
    ...
    // Kaon parameters
    totalCB->SetParLimits(7, 0, 8000);
    totalCB->FixParameter(8, beta_K_theory);
    totalCB->SetParLimits(9, 0.0025, 0.005);
    totalCB->SetParLimits(10, 0.1, 20.0);
    totalCB->SetParLimits(11, 1.0, 100.0);

    // Proton parameters
    totalCB->SetParLimits(12, 0, 30000);
    totalCB->FixParameter(13, beta_p_theory);
    totalCB->SetParLimits(14, 0.001, 0.006);
    totalCB->SetParLimits(15, 0.1, 20.0);
    ...
    double kaon_integral_after_dt = cb_kaon->Integral(0.9885, 1.0083) / binWidth;
    double kaon_integral_error_after_dt = cb_kaon->IntegralError(0.9885, 1.0083, totalCB->GetParameters(), fitResult->GetCovarianceMatrix().GetMatrixArray()) / binWidth;
    ...
    double proton_integral_after_dt = cb_proton->Integral(0.9881, 1.0083) / binWidth;
    double proton_integral_error_after_dt = cb_proton->IntegralError(0.9881, 1.0083, totalCB->GetParameters(), fitResult->GetCovarianceMatrix().GetMatrixArray()) / binWidth;
    ...
    double kaon_integral_after_chi2pid = cb_kaon->Integral(0.9879, 1.017) / binWidth;
    double kaon_integral_error_after_chi2pid = cb_kaon->IntegralError(0.9879, 1.017, totalCB->GetParameters(), fitResult->GetCovarianceMatrix().GetMatrixArray()) / binWidth;
    ...
    double proton_integral_after_chi2pid = cb_proton->Integral(0.984, 1.017) / binWidth;
    double proton_integral_error_after_chi2pid = cb_proton->IntegralError(0.984, 1.017, totalCB->GetParameters(), fitResult->GetCovarianceMatrix().GetMatrixArray()) / binWidth;
    ...
    // Output to file
    ofstream outFile("fit_parameters_bin5.txt");
    if (outFile.is_open()) {
        outFile << "Fit Parameters for Crystal Ball Fit (Bin: p = 2.5 - 2.8 GeV/c)\n";
        outFile << "------------------------------------\n";
        ...
        cerr << "Error: Cannot open fit_parameters_bin5.txt for writing" << endl;
    }
    ...
    hist_res->Draw();
    c_res->Print("residuals_bin5.png");

    canvas->Print("bin_5.png");
