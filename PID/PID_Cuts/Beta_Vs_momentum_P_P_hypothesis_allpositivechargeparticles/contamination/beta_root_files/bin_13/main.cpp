...
int main() {
    ...
    ROOT::RDF::TH1DModel modelBeta("beta_fit_cb", "p: [5.0-5.3) GeV/c; #beta; Counts", 200, 0.95, 1.03);
    ...
    Double_t initParams[21] = {
        ...
        amp_K, beta_K_theory, 0.002, 1.0, 2.0,             // Kaon (single-sided)
        amp_p, beta_p_theory, 0.002, 1.5, 3.0, 1.5, 2.0,   // Proton (independent double-sided)
    ...
    // Pion
    totalCB->SetParLimits(0, 0, 6000); totalCB->FixParameter(1, beta_pi_theory); totalCB->SetParLimits(2, 0.002, 0.0045);
    ...
    // Kaon
    totalCB->SetParLimits(7, 0, 1500); totalCB->FixParameter(8, beta_K_theory); totalCB->SetParLimits(9, 0.002, 0.002);
    totalCB->SetParLimits(10, 0.1, 50.0); totalCB->SetParLimits(11, 1.0, 20.0);

    // Proton
    totalCB->SetParLimits(12, 0, 2000); totalCB->FixParameter(13, beta_p_theory); totalCB->SetParLimits(14, 0.002, 0.0025);
   ...
   //totalCB->SetParameter(17, totalCB->GetParameter(15)); // Fix alphaRight to alphaLeft
   // totalCB->SetParameter(18, totalCB->GetParameter(16));

    // Background
    totalCB->SetParLimits(19, 0, 100); // a (intercept)
    totalCB->SetParLimits(20, -50, 50); // b (slope)
  ...
  */
   ...
   // Add individual proton peak contribution using the total function, including background
    ...
    ofstream outFile("fit_parameters_bin13.txt");
    if (outFile.is_open()) {
        ...
        cerr << "Error: Cannot open fit_parameters_bin13.txt for writing" << endl;
    }
