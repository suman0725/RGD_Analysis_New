...
int main() {
    ...
    ROOT::RDF::TH1DModel modelBeta("beta_fit_cb", "p: [1.9-2.2) GeV/c; #beta; Counts", 200, 0.95, 1.03);
    ...
    // Pion parameters
    totalCB->SetParLimits(0, 0, 180000);
    ...
    // Kaon parameters
    totalCB->SetParLimits(7, 0, 20000);
    ...
    double kaon_integral_after_dt = cb_kaon->Integral(0.9898, 1.008) / binWidth;
    double kaon_integral_error_after_dt = cb_kaon->IntegralError(0.9898, 1.008, totalCB->GetParameters(), fitResult->GetCovarianceMatrix().GetMatrixArray()) / binWidth;
    ...
    double kaon_integral_after_chi2pid = cb_kaon->Integral(0.981, 1.017) / binWidth;
    double kaon_integral_error_after_chi2pid = cb_kaon->IntegralError(0.981, 1.017, totalCB->GetParameters(), fitResult->GetCovarianceMatrix().GetMatrixArray()) / binWidth;
    ...
    // Output to file
    ofstream outFile("fit_parameters_bin3.txt");
    if (outFile.is_open()) {
        outFile << "Fit Parameters for Crystal Ball Fit (Bin: p = 1.9 - 2.2 GeV/c)\n";
        outFile << "------------------------------------\n";
        ...
        cerr << "Error: Cannot open fit_parameters_bin3.txt for writing" << endl;
    }
    ...
    hist_res->Draw();
    c_res->Print("residuals_bin3.png");

    canvas->Print("bin_3.png");
