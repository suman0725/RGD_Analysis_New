...
// Total function (π+ + kaons, no protons, no background)
...
int main() {
    ...
    TF1* totalCB = new TF1("totalCB", totalCrystalBallFunc, 0.95, 1.03, 10); // 10 parameters (5 pion, 5 kaon)
    totalCB->SetNpx(1000);
    totalCB->SetRange(0.95, 1.03);
    Double_t initParams[10] = {
    ...
    // Pion
    totalCB->SetParLimits(0, 0, 90000); totalCB->SetParameter(1, beta_pi_theory); totalCB->SetParLimits(2, 0.002, 0.004);
    totalCB->SetParLimits(3, 0.1, 10.0); totalCB->SetParLimits(4, 1.0, 20.0);
    // Kaon
    totalCB->SetParLimits(5, 0, 10000); totalCB->SetParameter(6, beta_K_theory); totalCB->SetParLimits(7, 0.0025, 0.004);
    ...
    TF1* cb_pion = new TF1("cb_pion", totalCrystalBallFunc, 0.95, 1.03, 10);
    cb_pion->SetNpx(1000);
    Double_t pionParams[10] = {0};
    for (int i = 0; i < 5; ++i) pionParams[i] = totalCB->GetParameter(i);
    for (int i = 5; i < 10; ++i) pionParams[i] = 0;
    cb_pion->SetParameters(pionParams);
    for (int i = 0; i < 10; ++i) cb_pion->FixParameter(i, pionParams[i]);
    cb_pion->SetLineColor(kBlue+2);
    cb_pion->SetLineWidth(2);
    cb_pion->Draw("SAME");

    // Kaon peak contribution
    TF1* cb_kaon = new TF1("cb_kaon", totalCrystalBallFunc, 0.95, 1.03, 10);
    cb_kaon->SetNpx(1000);
    Double_t kaonParams[10] = {0};
    for (int i = 5; i < 10; ++i) kaonParams[i] = totalCB->GetParameter(i);
    for (int i = 0; i < 5; ++i) kaonParams[i] = 0;
    cb_kaon->SetParameters(kaonParams);
    for (int i = 0; i < 10; ++i) cb_kaon->FixParameter(i, kaonParams[i]);
