...
int main() {
    ...
    for (int i = 0; i < nBins; i++) {
        ...
        // Filter for momentum range with chi2pid cut for pions
        ...
        // Filter for momentum range (uncut) for kaons
        ...
        histo_pions_cut->SetMarkerColor(kBlue);
        histo_pions_cut->Draw("P"); // Plot as points
        ...
        leg->AddEntry(histo_kaons.GetPtr(), 
                      TString::Format("Kaons Entries: %lld", (long long)histo_kaons->GetEntries()), "p");
        leg->SetBorderSize(0);
        leg->SetFillStyle(0);
