...
int main() {
    ...
    // Step 4: Set up momentum bins (4 to 7 GeV, width 0.3, 10 bins)
    vector<double> pBins;
    double pMin = 4.0, pMax = 7.0;
    int nBins = static_cast<int>((pMax - pMin) / 0.3); // 10 bins (0 to 9)
    ...
    // Step 7: Process each momentum bin (indices 0 to 9)
    for (int i = 0; i < nBins; i++) {
        ...
        // Define Gaussian fit for kaons with conditional ranges
        TF1* fitKaon = nullptr; // Declare outside the block to avoid scope issues
        if (histo_kaons->GetEntries() >= 10) {
            fitKaon = new TF1(TString::Format("fit_kaon_%d", i), "gaus", 0.96, 1.03); // Initial broad range
            if (i == 0) { // 4.0-4.3 GeV/c
                fitKaon->SetParameters(histo_kaons->GetMaximum(), 0.987, 0.01);
                histo_kaons->Fit(fitKaon, "R", "", 0.982, 0.993);
            } else if (i == 1) { // 4.3-4.6 GeV/c
                fitKaon->SetParameters(histo_kaons->GetMaximum(), 0.988, 0.01);
                histo_kaons->Fit(fitKaon, "R", "", 0.988, 0.995);
            } else if (i == 2) { // 4.6-4.9 GeV/c
                fitKaon->SetParameters(histo_kaons->GetMaximum(), 0.99, 0.1);
                histo_kaons->Fit(fitKaon, "R", "", 0.984, 0.992);
            } else if (i == 3) { // 4.9-5.2 GeV/c
                fitKaon->SetParameters(histo_kaons->GetMaximum(), 0.993, 0.01);
                histo_kaons->Fit(fitKaon, "R", "", 0.987, 0.994);
            } else if (i == 4) { // 5.2-5.5 GeV/c
                fitKaon->SetParameters(histo_kaons->GetMaximum(), 0.993, 0.01);
                histo_kaons->Fit(fitKaon, "R", "", 0.986, 0.995);
            } else if (i == 5) { // 5.5-5.8 GeV/c
                fitKaon->SetParameters(histo_kaons->GetMaximum(), 0.994, 0.1);
                histo_kaons->Fit(fitKaon, "R", "", 0.988, 0.996);
            } else if (i == 6) { // 5.8-6.1 GeV/c
                fitKaon->SetParameters(histo_kaons->GetMaximum(), 0.996, 0.012);
                histo_kaons->Fit(fitKaon, "R", "", 0.99, 0.996);
            } else if (i == 7) { // 6.1-6.4 GeV/c
                fitKaon->SetParameters(histo_kaons->GetMaximum(), 0.994, 0.012);
                histo_kaons->Fit(fitKaon, "R", "", 0.992, 0.997);
            } else if (i == 8) { // 6.4-6.7 GeV/c
                fitKaon->SetParameters(histo_kaons->GetMaximum(), 0.994, 0.012);
                histo_kaons->Fit(fitKaon, "R", "", 0.99, 0.998);
            } else if (i == 9) { // 6.7-7.0 GeV/c
                fitKaon->SetParameters(histo_kaons->GetMaximum(), 0.994, 0.1);
                histo_kaons->Fit(fitKaon, "R", "", 0.99, 0.998);
            } else {
                fitKaon->SetParameters(histo_kaons->GetMaximum(), 0.994, 0.01); // Default for unlisted bins
                histo_kaons->Fit(fitKaon, "R", "", 0.99, 1.0); // Default range
            }
        ...
        fitOutput << i << "\t" << pLow << "\t" << pHigh << "\t"
                  ...
                  << (fitKaon ? fitKaon->GetParameter(1) : 0) << "\t"
                  << (fitKaon ? fitKaon->GetParameter(2) : 0) << "\t"
                  << (fitKaon ? fitKaon->GetParameter(0) : 0) << "\t"
                  << (fitKaon ? fitKaon->GetChisquare() / fitKaon->GetNDF() : 0) << "\n";
