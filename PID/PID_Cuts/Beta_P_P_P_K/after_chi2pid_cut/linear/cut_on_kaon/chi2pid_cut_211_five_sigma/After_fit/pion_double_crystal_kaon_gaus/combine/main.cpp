...
pair<double, double> getKaonFitRange(double pMid) {
    if (pMid >= 4.0 && pMid < 4.3) return {0.99, 0.9962};   // 4.0-4.3 GeV/c
    if (pMid >= 4.3 && pMid < 4.6) return {0.992, 0.996};   // 4.3-4.6 GeV/c
    if (pMid >= 4.6 && pMid < 4.9) return {0.992, 0.997};   // 4.6-4.9 GeV/c
    if (pMid >= 4.9 && pMid < 5.2) return {0.993, 0.997};   // 4.9-5.2 GeV/c
    if (pMid >= 5.2 && pMid < 5.5) return {0.993, 0.998};
       // 5.2-5.5 GeV/c
    if (pMid >= 5.5 && pMid < 5.8) return {0.994, 0.998};   // 5.5-5.8 GeV/c
    if (pMid >= 5.8 && pMid < 6.1) return {0.99, 0.999};   // 5.8-6.1 GeV/c
    if (pMid >= 6.1 && pMid < 6.4) return {0.992, 0.998};   // 6.1-6.4 GeV/c
    if (pMid >= 6.4 && pMid < 6.7) return {0.992, 0.998};  // 6.4-6.7 GeV/c
    if (pMid >= 6.7 && pMid < 7.0) return {0.996, 0.998};   // 6.7-7.0 GeV/c
    // Added ranges for 1.0–4.0 GeV/c (placeholders, update later)
    if (pMid >= 1.0 && pMid < 1.3) return {0.96, 0.964};
    ...
    if (pMid >= 2.5 && pMid < 2.8) return {0.982, 0.99};
    if (pMid >= 2.8 && pMid < 3.1) return {0.983, 0.9915};
    if (pMid >= 3.1 && pMid < 3.4) return {0.985, 0.992};
    if (pMid >= 3.4 && pMid < 3.7) return {0.986, 0.9938};
    if (pMid >= 3.7 && pMid < 4.0) return {0.988, 0.996};
...
pair<double, double> getIntersectionRange(double pMid) {
    if (pMid >= 4.0 && pMid < 4.3) return {0.9955, 0.9957};   // Adjusted to match visual crossover
    if (pMid >= 4.3 && pMid < 4.6) return {0.994, 0.996};   // Adjusted to match visual crossover
    if (pMid >= 4.6 && pMid < 4.9) return {0.994, 0.997};   // Adjust based on visual inspection
    if (pMid >= 4.9 && pMid < 5.2) return {0.994, 0.997};   // Adjust based on visual inspection
    if (pMid >= 5.2 && pMid < 5.5) return {0.994, 0.998}; 
      // Adjust based on visual inspection
    if (pMid >= 5.5 && pMid < 5.8) return {0.995, 0.998};   // Adjust based on visual inspection
    if (pMid >= 5.8 && pMid < 6.1) return {0.995, 0.998};   // Adjust based on visual inspection
    if (pMid >= 6.1 && pMid < 6.4) return {0.995, 0.998};   // Adjust based on visual inspection
    if (pMid >= 6.4 && pMid < 6.7) return {0.995, 0.998};   // Adjust based on visual inspection
    if (pMid >= 6.7 && pMid < 7.0) return {0.995, 0.998};   // Adjust based on visual inspection // Adjust based on visual inspection
    // Placeholder ranges for 1.0–4.0 GeV/c (update later)
...
int main() {
    ...
    for (int i = 0; i < nBins; i++) {
        ...
        fitKaon->SetParameter(0, histo_kaons->GetMaximum());
        if (i == 3) {
            fitKaon->FixParameter(1, 0.9961);
            fitKaon->SetParameter(2, 0.003);
        } else if (i == 0) {
            ...
            fitKaon->FixParameter(1, 0.9966);
            fitKaon->SetParameter(2, 0.003);
        } else if (i == 5) {
            fitKaon->FixParameter(1, betaKaon);
            fitKaon->SetParameter(2, 0.005);
        } else if (i == 6) {
            fitKaon->FixParameter(1, betaKaon);
            fitKaon->SetParameter(2, 0.005);
        } else if (i == 7) {
            ...
            //fitKaon->FixParameter(1, betaKaon);
            fitKaon->SetParameter(1, betaKaon);
            fitKaon->SetParameter(2, 0.005);
        } else if (i == 17) {
            //fitKaon->FixParameter(1, betaKaon);
            fitKaon->SetParameter(1, betaKaon);
        ...
        if (histo_pions->GetEntries() >= 10 && histo_kaons->GetEntries() >= 10) {
            intersectionPoint = findIntersection(fitKaon, fitPion, pMid);
