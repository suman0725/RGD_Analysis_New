...
int main() {
    ...
    // Step 5: Define chi2pid cut constants
    const double C = 0.9795; // Scaling factor
    ...
    canvas->Print("output/beta_plots/beta_distributions_kaon_chi2cut_linear.pdf[");
    ...
    for (int i = 0; i < nBins; i++) {
        ...
        // Update and save
        canvas->Update();
        canvas->Print(TString::Format("output/beta_plots/beta_distributions_kaon_chi2cut_linear.pdf"));
        delete leg;
    }
