...
int main() {
    ...
    // Step 5: Define chi2pid cut constants for pions
    const double C = 0.9795; // Scaling factor
    ...
    canvas->Print("output/beta_plots/beta_distributions_no_kaon_chi2cut_linear.pdf[");

    // Step 7: Process each momentum bin
    for (int i = 0; i < nBins; i++) {
        double pLow = pBins[i], pHigh = pBins[i + 1];

        // Filter for momentum range and apply chi2pid cut for pions only
        ...
        ROOT::RDF::TH1DModel modelBetaPions(
            ...
            100, 0.96, 1.02 // Beta range set to 0.95-1.03
        );
        ...
        leg->AddEntry(histo_pions.GetPtr(), "Pions (chi2pid cut)", "p");
        leg->AddEntry(histo_kaons.GetPtr(), "Kaons (no chi2pid cut)", "p");
        leg->SetBorderSize(0);
        leg->SetFillStyle(0);
        ...
        // Update and save
        canvas->Update();
        canvas->Print(TString::Format("output/beta_plots/beta_distributions_no_kaon_chi2cut_linear.pdf"));
        delete leg;
    }
