...
int main() {
    ...
    /* TH1F* h1_deltaT_deuteron_wpid = new TH1F("h1_deltaT_deuteron_wpid", "Deuteron; #DeltaT (ns); Counts", 100, -50, 150);
    TH1F* h1_deltaT_deuteron_wopid = new TH1F("h1_deltaT_deuteron_wopid", "Deuteron; #DeltaT (ns); Counts", 100, -50, 150); */

    TH1F* h1_deltaT_antiproton_wpid = new TH1F("h1_deltaT_antiproton_wpid", "antiproton; #DeltaT (ns); Counts", 100, -50, 150);
    TH1F* h1_deltaT_antiproton_wopid = new TH1F("h1_deltaT_antiproton_wopid", "antiproton; #DeltaT (ns); Counts", 100, -50, 155);
    ...
    const float mass_antiproton = 0.93827f;
    ...
    int count_all_negitve = 0; 
    int count_negative_pion = 0; 
    int count_negative_kaon = 0; 
    int count_antiproton = 0 ; 
        ...
        for (const auto& file : hipoFiles) {
            ...
            while (reader.next() == true ) {
               ...
               for (int i = 0; i < PART.getRows(); i++) {
                    ...
                    //float startTime = PART.getFloat("startTime",i);
                    ...
                    if (best_layer == -1 && caloMap.find(i) != caloMap.end()) {
                        int bestEcalLayer = -1;
                        for (int iCaloRow : caloMap[i]) {
                            if (CALO.getByte("detector", iCaloRow) == 7) {
                                int layer = CALO.getByte("layer", iCaloRow);
                                if ((layer == 1 && bestEcalLayer < 1) || (layer == 4 && bestEcalLayer < 4) || (layer == 7 && bestEcalLayer < 7)) {
                                    bestEcalLayer = layer;
                                    best_path = CALO.getFloat("path", iCaloRow);
                                    best_time = CALO.getFloat("time", iCaloRow);
                                }
                            }
                        }
                        if (bestEcalLayer != -1) best_layer = bestEcalLayer;
                        cout << "Found ECAL for EB" << i << endl; 
                    }
                    ...
                    if (best_layer != -1) {
                        ...
                        else if (bank_pid == -2212) mass = mass_antiproton;
                        //else if (bank_pid == 45) mass = mass_deuteron;
                        float dt= calculateDeltaT(best_path, best_time, calculateBeta(p, mass), startTime); 
                        
                        if (bank_pid == -211) {
                            h1_deltaT_pion_negative_wpid->Fill(dt);
                            count_negative_pion++;
                        }
                        else if (bank_pid == -321) {
                            count_negative_kaon++; 
                            h1_deltaT_kaon_negative_wpid->Fill(dt);
                        }
                        else if (bank_pid == -2212) {
                            count_antiproton++; 
                            h1_deltaT_antiproton_wpid->Fill(dt);
                        }
                        /* else if (bank_pid == 45) {
                            count_deuteron++; 
                   ...
                   // --- Manual PID ΔT calculation with FTOF panel prioritization (by LAYER) ---
                    ...
                    // Now calculate ΔT using the selected path and time only if the beta is negative
                    float beta_pion = calculateBeta(p, mass_pion);
                    float beta_kaon = calculateBeta(p, mass_kaon);
                    float beta_antiproton = calculateBeta(p, mass_antiproton);
                 ...
                 // If no valid hits found, skip the particle
                    ...
                    // Only proceed if beta values are negative
                    if (beta_pion > 0) {
                    ...
                    if (beta_antiproton > 0) {
                        dt_antiproton = calculateDeltaT(selectedPath, selectedTime, beta_antiproton, startTime);
                    }
                    ...
                    best_dt_antiproton = abs(dt_antiproton);
                    ...
                    if (best_dt_antiproton < min_dt) {
                        manual_selected_pid = -2212;
                        min_dt = best_dt_antiproton;
                    }
                    ...
                    if (manual_selected_pid == -211 && passes_cherenkov ) {
                        h1_deltaT_pion_negative_wopid->Fill(dt_pion);  
                        count_manual++;
                    }
                    if (manual_selected_pid == -321 && passes_cherenkov ) {
                        h1_deltaT_kaon_negative_wopid->Fill(dt_kaon);  
                        //count_manual++;
                    }
                    if (manual_selected_pid == -2212 && passes_cherenkov ) {
                        h1_deltaT_antiproton_wopid->Fill(dt_antiproton); 
                        //count_manual++;
                    }
                    /* if (manual_selected_pid == 45 ) {
                ...
                //all_sum = count_negative_pion + count_negative_kaon + count_antiproton + count_deuteron + count_electron + count_pidzero ; 
                all_sum = count_negative_pion + count_negative_kaon + count_antiproton /* + count_deuteron */ + count_electron  ; 
   ...
   leg1->SetTextSize(0.02); 
    ...
    c1->cd(3);
    h1_deltaT_antiproton_wopid->SetLineColor(kRed);
    h1_deltaT_antiproton_wpid->SetLineColor(kBlue);
    h1_deltaT_antiproton_wopid->Draw();
    h1_deltaT_antiproton_wpid->Draw("SAME");

    TLegend *leg2 = new TLegend(0.7, 0.7, 0.9, 0.9);
    leg2->SetTextSize(0.02); 
    leg2->AddEntry(h1_deltaT_antiproton_wopid, 
                Form("Custom PID (%.0f)", h1_deltaT_antiproton_wopid->GetEntries()), 
                "l");
    leg2->AddEntry(h1_deltaT_antiproton_wpid, 
                Form("EB PID (%.0f)", h1_deltaT_antiproton_wpid->GetEntries()), 
                "l");
    leg2->Draw();
    ...
    c1->SaveAs("deltaT_comparison_018536_neg.pdf");
    ...
    TCanvas *c3 = new TCanvas("c3", "DeltaT Comparison antiproton", 800, 600);
    c3->Divide(2, 2);

    // First plot
    c3->cd(1);
    //h1_deltaT_deuteron_wpid->GetXaxis()->SetRangeUser(-10, 10); // Set range before drawing
    h1_deltaT_antiproton_wpid->Draw(); 
    ...
    // Save the canvas
    c3->SaveAs("deltaT_comparison_018536_antiproton_neg.pdf");
    ...
    cout << "****************************************************************************************" << endl; 
    cout << "Total number -ve charge particles after charge cut: " << count_all_negitve << endl; 
    cout << "Total number of  -ve pions with pid -211: " << count_negative_pion << endl; 
    cout << "Total number of  -ve kaons with pid -321: " << count_negative_kaon << endl; 
    cout << "Total number of antiprotons with pid -2212: " << count_antiproton << endl; 
    cout << "Total number of  deuterons with pid 45: " << count_deuteron << endl; 
    cout << "Total number of electrons with pid 11: " << count_electron << endl; 
    ...
    cout << "Total electron: " << electron_count << endl; 
