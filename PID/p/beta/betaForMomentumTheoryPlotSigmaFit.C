...
void betaForMomentumTheoryPlotSigmaFit() {
    TStopwatch timer;
    timer.Start();
    ...
    // Create a new TGraph for fitting the mean ± 3σ points
    TGraph* pionDataFit = new TGraph(2 * pion_means.size());

    // Fill pionDataFit with mean ± 3σ points
    for (size_t i = 0; i < pion_means.size(); ++i) {
        double mean = pion_means[i];
        double sigma = pion_sigmas[i];
        double lowerBound = mean - 3 * sigma;
        ...
        pionDataFit->SetPoint(2 * i, momentum_ranges[i], lowerBound); // mean - 3σ
        pionDataFit->SetPoint(2 * i + 1, momentum_ranges[i], upperBound); // mean + 3σ
    }

    // Set up the fit function with initial parameters
    TF1* fitFunction = new TF1("fitFunction", "([0]*x)/sqrt([1]*x*x + [2] + [3]) + [4]", 0, 8);

    // Optionally, you can set initial guesses for the parameters
    fitFunction->SetParameters(1, 1, 1, 1, 1); // Adjust as needed based on your data

    // Fit the pionDataFit graph
    pionDataFit->Fit(fitFunction, "R"); // "R" is for range, fit over the full x-range

    // Draw the fit
    fitFunction->Draw("SAME"); // Overlay the fit on the existing plot


    canvas->SaveAs("output_with_all_curves_and_pion_data_points_fit.pdf");
